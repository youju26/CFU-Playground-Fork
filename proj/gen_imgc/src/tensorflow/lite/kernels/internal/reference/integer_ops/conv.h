    ...
    for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
      for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
        ...
        for (; in_channel + 3 < filter_input_depth; in_channel += 4) {
          ...
          CFU_MAC_SET_FILTER_VALS(filter_val);
        }
        ...
        for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
            ...
            for (int filter_x = 0; filter_x < filter_width; ++filter_x) {
              ...
              if (!is_point_inside_image) {
                int in_channel = 0;
                for (; in_channel + 3 < filter_input_depth; in_channel += 4) {
                  CFU_MAC_SET_INPUT_VALS(packed_input_zero_point, 0);
                }
                for (; in_channel < filter_input_depth; ++in_channel) {
              ...
              const int8_t* input_ptr = input_data +
                                        Offset(input_shape, batch, in_y, in_x,
                                               0);
              for (; in_channel + 3 < filter_input_depth;
                   in_channel += 4, input_ptr += 4) {
                uint32_t input_val = *((const uint32_t*)(input_ptr));
